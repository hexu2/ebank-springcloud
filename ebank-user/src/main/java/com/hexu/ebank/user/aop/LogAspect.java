package com.hexu.ebank.user.aop;
...
public class LogAspect {
    ...
    @Autowired
    private TUserLogService tUserLogObjService;
    ...
    public void before(JoinPoint joinPoint) {
        ...
        if(!StringUtils.isEmpty(uuid)){
            tUserLog.setUsername(uuid);
            tUserLog.setCreateTime(new Date());
            int impactcount = tUserLogObjService.insert(tUserLog);
            logger.info("成功记录用户操作记录条数 impact count ： value = {}", impactcount);
        }
...
//            tUserLog.setCreateTime(new Date());
//            tUserLogObjService.insert(tUserLog);
//        }
    }
